...
def create_test_datasets(
    ...
    image_names  = [image_name for image_name in image_names if image_name.endswith(file_ending)]
    ...
    val_images = [image_name for image_name in image_names if _return_true_if_string_in_list_set(image_name, val_segs)]
    print(f"Moving {len(val_segs)} out {len(val_segs)+len(seg_names)} Label Maps to Validation Data")
...
def move_files(source_dir:Path, target_dir:Path, file_names: list[str], file_ending:str):
    os.makedirs(target_dir, exist_ok=False)
    for filename in file_names:
        file_name = filename+file_ending
        shutil.move(source_dir/ file_name, target_dir / file_name)

def copy_files(source_dir:Path, target_dir:Path, file_names: list[str], file_ending:str):
    os.makedirs(target_dir, exist_ok=False)
    for filename in file_names:
        file_name = filename+file_ending
...
if __name__ == "__main__":
    ...
    create_test_datasets(labelsTr, imagesTr, labelsVal, imagesVal, file_ending, test_size=test_size)