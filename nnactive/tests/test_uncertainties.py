...
from nnactive.strategies.uncertainties import (
    prob_exp_entropy,
    prob_mutual_information,
    prob_pred_entropy,
)
...
def test_pred_entropy():
    ...
    for i in range(len(probs)):
        entropies[i] = prob_pred_entropy(probs[i], device=torch.device("cpu"))
    ...
    for i in range(len(probs_05)):
        entropies[i] = prob_pred_entropy(probs_05[i], device=torch.device("cpu"))
        assert np.isclose(entropies[i].item(), -np.log(0.5))


def test_exp_entropy():
    probs, probs_start = probs_two_class()
    ...
    for i in range(len(probs)):
        exp_entropies[i] = prob_exp_entropy(probs_ident[i], device=torch.device("cpu"))
        assert np.isclose(
            exp_entropies[i].item(),
            prob_exp_entropy(probs_ident[i], device=torch.device("cpu")).item(),
        )
    ...
    for i in range(len(probs_05)):
        assert (
            prob_exp_entropy(probs_05[i], device=torch.device("cpu"))
            <= prob_pred_entropy(probs_05[i], device=torch.device("cpu"))
        ).item()
...
def test_mutual_information():
    ...
    for i in range(len(probs)):
        assert np.isclose(
            0,
            prob_mutual_information(probs_ident[i], device=torch.device("cpu")).item(),
        )
    ...
    for i in range(len(probs_05)):
        mutual_informations[i] = prob_mutual_information(
