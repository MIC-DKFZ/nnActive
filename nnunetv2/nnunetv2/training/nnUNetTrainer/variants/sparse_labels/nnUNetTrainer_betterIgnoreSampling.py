            ...
            if voxels_of_that_class is not None:
                ...
                if self.has_ignore and not force_fg:
                    ...
                    allowed_max_neg_offset = [
                        min(s, p // 2)
                        for s, p in zip(selected_voxel[1:], self.patch_size)
                    ]
                    allowed_max_pos_offset = [
                        min(d - s, p // 2)
                        for s, p, d in zip(
                            selected_voxel[1:], self.patch_size, data_shape
                        )
                    ]
                    for d in range(len(self.patch_size)):
                        selected_voxel[d + 1] += np.random.randint(
                            -allowed_max_neg_offset[d], allowed_max_pos_offset[d]
                        )
...
class nnUNetDataLoader2DBetterIgnSampling(nnUNetDataLoader2D):
    def get_bbox(
        ...
        return nnUNetDataLoaderBaseBetterIgnSampling.get_bbox(
            self, data_shape, force_fg, class_locations, overwrite_class, verbose
        )
