...
class InhomogeneousSliceIlluminationTransform(AbstractTransform):
    def __init__(
        ...
        num_defects,
        defect_width,
        mult_brightness_reduction_at_defect,
        base_p,
    ...
    def _build_defects(self, num_slices):
        ...
        ps += (
            1 - int_factors
        ) / num_slices  # probability in defect areas is twice as high as in the rest
        ps /= ps.sum()
    ...
    def __call__(self, **data_dict):
        ...
        assert (
            len(data.shape) == 5
        ), "this only works on 3d images, the provided tensor is 4d, so it's a 2d image (bcxy)"
...
class LowContrastTransform(AbstractTransform):
    def __init__(
        self,
        constrast_reduction: Union[Tuple[float, float], Callable],
        per_channel: bool,
        p_per_sample: float,
        p_per_channel: float,
        data_key: str = "data",
            ...
            if self.p_per_sample > np.random.uniform():
                ...
                for c in range(data.shape[1]):
                    if self.p_per_channel > np.random.uniform():
                        ...
                        data[b, c] -= mn
                        mult_here = (
                            self._get_mult_for_contrast() if mult is None else mult
                        )
                        data[b, c] *= mult_here
                        data[b, c] += mn
        data_dict[self.data_key] = data
