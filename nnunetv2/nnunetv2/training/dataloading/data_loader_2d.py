...
class nnUNetDataLoader2D(nnUNetDataLoaderBase):
    def generate_train_batch(self):
        ...
        for j, current_key in enumerate(selected_keys):
            ...
            if (
                selected_class_or_region is not None
                and len(properties["class_locations"][selected_class_or_region]) > 0
            ):
                selected_slice = properties["class_locations"][
                    selected_class_or_region
                ][
                    np.random.choice(
                        properties["class_locations"][selected_class_or_region].shape[1]
                    )
                ][
                    1
                ]
