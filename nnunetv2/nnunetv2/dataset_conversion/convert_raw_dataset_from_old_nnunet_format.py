...
def convert(source_folder, target_dataset_name):
    ...
    shutil.copytree(
        ...
        join(paths.nnUNet_raw, target_dataset_name, "imagesTr"),
    )
    shutil.copytree(
        join(source_folder, "labelsTr"),
        join(paths.nnUNet_raw, target_dataset_name, "labelsTr"),
    )
    if isdir(join(source_folder, "imagesTs")):
        shutil.copytree(
            join(source_folder, "imagesTs"),
            join(paths.nnUNet_raw, target_dataset_name, "imagesTs"),
        )
    if isdir(join(source_folder, "labelsTs")):
        shutil.copytree(
            join(source_folder, "labelsTs"),
            join(paths.nnUNet_raw, target_dataset_name, "labelsTs"),
        )
    if isdir(join(source_folder, "imagesVal")):
        shutil.copytree(
            join(source_folder, "imagesVal"),
            join(paths.nnUNet_raw, target_dataset_name, "imagesVal"),
        )
    if isdir(join(source_folder, "labelsVal")):
        shutil.copytree(
            join(source_folder, "labelsVal"),
            join(paths.nnUNet_raw, target_dataset_name, "labelsVal"),
        )
    shutil.copy(
        join(source_folder, "dataset.json"), join(paths.nnUNet_raw, target_dataset_name)
    )

    dataset_json = load_json(
        join(paths.nnUNet_raw, target_dataset_name, "dataset.json")
    )
