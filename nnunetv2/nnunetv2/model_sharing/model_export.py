...
def export_pretrained_model(
    ...
    with zipfile.ZipFile(output_file, "w", zipfile.ZIP_DEFLATED) as zipf:
        for c in configurations:
            ...
            for fold_folder in expected_fold_folder:
                ...
                if isfile(source_file):
                    zipf.write(
                        source_file, os.path.relpath(source_file, paths.nnUNet_results)
                    )
                ...
                if export_crossval_predictions:
                    ...
                    for f in files:
                        zipf.write(
                            ...
                            os.path.relpath(
                                join(source_folder, f), paths.nnUNet_results
                            ),
                        )
        ...
        inference_information_file = join(
            paths.nnUNet_results, dataset_name, "inference_information.json"
        )
        if isfile(inference_information_file):
            zipf.write(
                inference_information_file,
                os.path.relpath(inference_information_file, paths.nnUNet_results),
            )
        inference_information_txt_file = join(
            paths.nnUNet_results, dataset_name, "inference_information.txt"
        )
        if isfile(inference_information_txt_file):
            zipf.write(
                inference_information_txt_file,
                os.path.relpath(inference_information_txt_file, paths.nnUNet_results),
            )
    print("Done")
