...
def load_softmax_predictions(softmax_file_name, pred_path):
    ...
    n_preds = 5
    fold_0_image = torch.from_numpy(np.load(os.path.join(pred_path, "fold_0", softmax_file_name))['probabilities.npy'])
    softmax_preds = torch.zeros(
        n_preds,
        *fold_0_image.shape
    )
    ...
    for fold in range(1,n_preds):
        fold_image = torch.from_numpy(np.load(os.path.join(pred_path, f"fold_{fold}", softmax_file_name))['probabilities.npy'])
...
def get_pred_image_info(softmax_file_name, pred_path):
    ...
    pred_path = os.path.join(fold_0_path, f"{softmax_file_name.split('.')[0]}{file_ending_pred}")
...
def calculate_uncertainties(softmax_file_names, pred_path):
    ...
    for image_name in softmax_file_names:
        ...
        pred_origin, pred_spacing, pred_direction = get_pred_image_info(image_name, pred_path)
        
        # set the correct image information for the uncertainty maps
        for image in [pred_entropy_image, expected_entropy_image, mutual_information_image]:
        ...
        sitk.WriteImage(pred_entropy_image, os.path.join(pred_path, "uncertainties", f"{image_name.split('.')[0]}_pred_entropy.nii.gz"))
        sitk.WriteImage(expected_entropy_image, os.path.join(pred_path, "uncertainties", f"{image_name.split('.')[0]}_expected_entropy.nii.gz"))
        sitk.WriteImage(mutual_information_image, os.path.join(pred_path, "uncertainties", f"{image_name.split('.')[0]}_mutual_information.nii.gz"))
